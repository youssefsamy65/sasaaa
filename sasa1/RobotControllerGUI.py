...
def open_main_window():
    ...
    tk.Label(btn_frame4, text="manual control", font=("Arial", 12)).pack()
...
def start_chatbot(language):
    main_window.withdraw()
    if language == "Arabic":
    	subprocess.run(["/home/oem/sasa1/arabic.sh"])
    else:
    	subprocess.run(["/usr/bin/python3", "/home/oem/sasa/GeminiEnglish.py"])



def option2_clicked():
    main_window.withdraw()
    url = "http://192.168.50.250:3000/Map"
    webbrowser.open_new_tab(url)
